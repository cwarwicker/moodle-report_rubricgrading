 ...
 *
 * Covers the per-criterion/per-level data stored in:
 *   gradingform_rubric_fillings  (the grader's filling: which level, remark)
 *   gradingform_rubric_criteria  (criterion description, sort order)
 *   gradingform_rubric_levels    (level score and definition text)
 *
...
class rubric_criterion extends base {
    ...
    protected function get_default_entity_title(): lang_string {
        return new lang_string('rubriccriterion', 'report_rubricgrading');
    }
    ...
    protected function get_all_columns(): array {
        ...
        // Score value of the selected rubric level.
        $columns[] = (new column(
            'score',
            new lang_string('score', 'report_rubricgrading'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
        ...
        // Definition text of the selected level.
        $columns[] = (new column(
            'leveldefinition',
            new lang_string('definition', 'report_rubricgrading'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
        ...
        // Per-criterion remark left by the grader.
        $columns[] = (new column(
            'remark',
            new lang_string('feedback', 'report_rubricgrading'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_LONGTEXT)
